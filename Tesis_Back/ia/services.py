...
def build_case_summary_prompt(ctx: dict) -> str:
    return (
        "Eres un abogado senior especializado en redactar informes ejecutivos para que un colega pueda hacer un 'refresh' rápido del estado de una causa o presentársela a su jefe. "
        "Tu tarea es redactar un informe narrativo completo y útil de la causa judicial, basándote ESTRICTAMENTE en los datos del JSON proporcionado.\n\n"
        ...
        "3. **Próximas actuaciones:** Menciona los eventos futuros con sus fechas y qué implican. Si no hay ninguno, indícalo.\n\n"
        "Reglas:\n"
        "- No inventes información. Si un dato no está en el JSON, omítelo o escribe 'no consta'.\n"
        "- Usa lenguaje técnico-legal claro, directo y profesional.\n"
        "- Las descripciones de eventos son tu fuente principal para entender QUÉ pasó en la causa, aprovéchalas al máximo.\n"
        "- La carátula tiene el formato 'APELLIDO c/ EMPRESA s/ TIPO_ACCION'; el campo 's/' indica el tipo de acción judicial.\n"
        "- No uses listas, viñetas ni títulos Markdown. Solo párrafos de texto corrido.\n\n"
